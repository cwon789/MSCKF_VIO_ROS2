...
The ROS 2 Foxy port has been tested on Ubuntu 20.04.
...
## Building (ROS 2 Foxy)
Build with colcon after cloning into your ROS 2 workspace:

```
cd your_work_space
colcon build --packages-select msckf_vio --cmake-args -DCMAKE_BUILD_TYPE=Release
...
Once the package is built and sourced (via `source install/setup.bash`), there are ROS 2 launch files prepared for the [EuRoC](https://projects.asl.ethz.ch/datasets/doku.php?id=kmavvisualinertialdatasets) and [UPenn fast flight](https://github.com/KumarRobotics/msckf_vio/wiki/Dataset) dataset named `msckf_vio_euroc.launch.py` and `msckf_vio_fla.launch.py` respectively. Each launch file instantiates two ROS 2 nodes:
...
Once the nodes are running, play back the dataset bag (e.g. converted to rosbag2) in another terminal, for example:
