...
bool ImageProcessor::loadParameters() {
  ...
  RCLCPP_INFO(get_logger(), "grid_row: %d",
      processor_config.grid_row);
  RCLCPP_INFO(get_logger(), "grid_col: %d",
...
void ImageProcessor::addNewFeatures() {
  ...
  if (matched_new_features < 5 &&
    ...
    RCLCPP_WARN(get_logger(), "Images at [%f] seems unsynced...",
        toSec(cam0_curr_img_ptr->header.stamp));
...
void ImageProcessor::undistortPoints(
  ...
  if (distortion_model == "radtan") {
    ...
    RCLCPP_WARN_ONCE(get_logger(),
                  "The model %s is unrecognized, use radtan instead...",
...
vector<cv::Point2f> ImageProcessor::distortPoints(
  ...
  if (distortion_model == "radtan") {
    ...
    RCLCPP_WARN_ONCE(get_logger(),
                  "The model %s is unrecognized, using radtan instead...",
                  distortion_model.c_str());
    vector<cv::Point3f> homogenous_pts;
    cv::convertPointsToHomogeneous(pts_in, homogenous_pts);
    cv::projectPoints(homogenous_pts, cv::Vec3d::zeros(), cv::Vec3d::zeros(), K,
...
void ImageProcessor::integrateImuData(
  ...
  while (begin_iter != imu_msg_buffer.end()) {
    if ((toSec(begin_iter->header.stamp) -
          toSec(cam0_prev_img_ptr->header.stamp)) < -0.01)
      ++begin_iter;
    else
      break;
  }
  ...
  while (end_iter != imu_msg_buffer.end()) {
    if ((toSec(end_iter->header.stamp) -
          toSec(cam0_curr_img_ptr->header.stamp)) < 0.005)
      ++end_iter;
    else
      break;
  }
  ...
  double dtime = toSec(cam0_curr_img_ptr->header.stamp) -
      toSec(cam0_prev_img_ptr->header.stamp);
...
void ImageProcessor::twoPointRansac(
  ...
  if (pts1.size() != pts2.size())
    RCLCPP_ERROR(get_logger(), "Sets of different size (%lu and %lu) are used...",
        pts1.size(), pts2.size());
